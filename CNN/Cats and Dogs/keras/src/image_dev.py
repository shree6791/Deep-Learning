...
class ImageDataGenerator(object):
    ...
    def random_transform(self, x, seed=None):
        ...
        # Define Custom Blurring Parameter Using Gaussian Filter For Image Preprocessing
        if self.blurring:

            if np.shape(x)[2] == 1: 
                x = gaussian(x, sigma=self.blurring, multichannel=None)

            elif np.shape(x)[2] == 3: 
                x = gaussian(x, sigma=self.blurring, multichannel=False)
